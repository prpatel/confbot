...
public class ConferenceTools {
    ...
    @Tool(description = "Lists all sessions for a specific speaker. This will find all the sessions a person has at the conference.")
    ...
    public String sessionInfo(String title) {
        logger.info("\n\ninvoking sessionInfo():" + title);
        List<Session> sessions = sessionRepository.findByTitle(title);
        logger.info("\nsessionInfo() found this many sessions:" + sessions.size());
        if (sessions.isEmpty()) {
        ...
        try {
            for (Session session : sessions) {
                result.append("Title: ").append(session.getTitle() != null ? session.getTitle() : "N/A").append("\n")
                        .append("Description: ").append(session.getDescription() != null ? session.getDescription() : "N/A").append("\n")
                        .append("Room: ").append(session.getRoom() != null ? session.getRoom() : "N/A").append("\n")
                        .append("Time: ").append(session.getStartsAt() != null ? session.getStartsAt() : "N/A").append(" - ").append(session.getEndsAt() != null ? session.getEndsAt() : "N/A").append("\n")
                        .append("Speakers: ").append(session.getSpeakers() != null ? session.getSpeakers().stream().map(Speaker::getFullName).collect(Collectors.joining(", ")) : "N/A").append("\n\n");
            }
        } catch (Exception e) {
            logger.error("\nsessionInfo() ERROR ERROR ERROR Issue parsing Session information", e);
            return " ERROR Issue parsing Session information!";
        }

        logger.info("\nsessionInfo() returned this session:"+result);
        return result.toString();
    }
    ...
    @Tool(description = "Finds speakers from a specific company. This searches the database all the people from a company.")
    ...
    @Tool(description = "Finds the company of a specific speaker. This tool will find the company of a person")
